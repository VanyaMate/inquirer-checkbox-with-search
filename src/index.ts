...
const normalizeChoices = function <Value> (
...
const filterItem = function <Value> (item: Item<Value>, value: string): boolean {
...
const notSeparator = function <Value> (item: Item<Value>): item is NormalizedChoice<Value> {
    return !Separator.isSeparator(item);
};

// Переключение элемента
const toggle = function <Value> (item: Item<Value>) {
    if (!Separator.isSeparator(item)) {
...
// Переключение элемента
const check = function <Value> (item: Item<Value>) {
    if (!Separator.isSeparator(item)) {
        item.checked = true;
    }
...
// Переключение элемента
const uncheck = function <Value> (item: Item<Value>) {
    if (!Separator.isSeparator(item)) {
        item.checked = false;
    }
...
const checked = function <Value> (item: Item<Value>) {
    if (Separator.isSeparator(item)) {
        return false;
    }
...
const value = function <Value> (item: Item<Value>) {
    if (Separator.isSeparator(item)) {
...
const toggleByIndex = function <Value> (activeIndex: number) {
    return function (item: Item<Value>, index: number) {
...
const getActiveIndex = function (currentIndex: number, delta: number, length: number) {
    return (currentIndex + delta + length) % length;
};

// Функция рендера элемента списка
const renderRow = function <Value> (
    props: {
        isActive: boolean,
        item: Item<Value>
    },
...
export default createPrompt(
    <Value> (config: CheckboxWithSearchConfig<Value>, done: (value: Array<Value>) => void) => {
        const {
                  message,
                  pageSize = 7,
                  loop     = true,
        ...
        // Тема для чекбоксов
        const checkboxTheme = makeTheme<CheckboxTheme>(defaultCheckboxTheme, theme);

        // Тема для ввода
        const inputTheme = makeTheme<InputTheme>();
        ...
        if (status === 'done') {
            const selectedItems = checkboxTheme.style.highlight(
                checkboxTheme.style.renderSelectedChoices(
                    originalItems.filter(notSeparator).filter(checked), originalItems,
                ),
            );
